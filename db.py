...
-- Una sola fila: cómo fue el último ciclo. Sirve para que una web vacía
-- pueda explicar POR QUÉ está vacía en vez de dejarte a ciegas.
...
class DB:
    ...
    def anotar_error(self, dominio: str, error: str) -> None:
        """Por qué falló esta tienda. Un fallo silencioso no se puede arreglar.

        La tienda puede no estar ni registrada todavía (si el reconocimiento
        también falló), así que si no existe la fila se crea.
        """
        with self.cursor() as cur:
            cur.execute("""INSERT INTO tiendas (dominio, plataforma, ultimo_error,
                                                descubierta)
                           VALUES (?, NULL, ?, ?)
                           ON CONFLICT(dominio) DO UPDATE SET
                             ultimo_error = excluded.ultimo_error""",
                        (dominio, error[:200], datetime.utcnow().isoformat()))

    def errores_frecuentes(self, limite: int = 6) -> list[dict]:
        with self.cursor() as cur:
            cur.execute("""SELECT ultimo_error AS error, COUNT(*) AS n,
                                  MIN(dominio) AS ejemplo
                           FROM tiendas
                           WHERE ultimo_error IS NOT NULL AND ultimo_error <> ''
                           GROUP BY ultimo_error ORDER BY n DESC LIMIT ?""",
                        (limite,))
    ...
    def guardar_diagnostico(self, datos: dict) -> None:
        with self.cursor() as cur:
            cur.execute("""INSERT INTO diagnostico (id, cuando, datos)
                           VALUES (1, ?, ?)
                           ON CONFLICT(id) DO UPDATE SET
                             cuando = excluded.cuando, datos = excluded.datos""",
                        (datetime.utcnow().isoformat(), json.dumps(datos)))

    def diagnostico(self) -> dict:
        with self.cursor() as cur:
            cur.execute("SELECT cuando, datos FROM diagnostico WHERE id = 1")
            r = cur.fetchone()
        if not r:
            return {}
        try:
