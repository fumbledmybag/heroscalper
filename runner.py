...
class HeroScalper:
    ...
    async def descubrir(self, dominios: list[str]) -> list[dict]:
        ...
        for dominio, h in zip(dominios, huellas):
            ...
            if h.vigilable:
                ...
                self.db.anotar_error(h.dominio, "")
            else:
                self.db.anotar_error(
                    h.dominio,
                    "no responde" if not h.alcanzable
                    else "responde, pero sin plataforma reconocible ni JSON-LD")
        return resultados
    ...
    async def barrer_tienda(self, dominio: str, plataforma: Optional[str] = None,
        ...
        try:
            ...
            motivo = f"{type(e).__name__}: {censurar(e, os.environ.get('KEEPA_API_KEY'))}"
            self.db.anotar_error(dominio, motivo)
        ...
        if ofertas:
            ...
            self.db.anotar_error(dominio, "")
            cambiadas = self.db.guardar_lecturas(ofertas)
        else:
            # Ni excepción ni productos. Puede ser que no conteste, que nos
            # eche, o que conteste bien y no sepamos leerla: son problemas
            # distintos y hasta ahora los tres se veían igual (o sea, nada).
            motivo = self.cliente.ultimo_fallo.get(dominio)
            self.db.anotar_error(
                dominio, motivo or "responde, pero no le encontramos el catálogo")
    ...
    async def tasar_candidatas(self, oportunidades: list,
        ...
        for op in oportunidades:
            if hechas >= tope:
                break
            # Vale tanto una Oportunidad ya evaluada como una Oferta suelta:
            # ahora se tasa ANTES de evaluar, para que haya con qué evaluar.
            oferta = getattr(op, "oferta", op)
            clave = oferta.clave_match()
            if clave in vistas or self.db.cotizacion_fresca(clave, horas):
                continue
            vistas.add(clave)

            cotizaciones = await self.tasador.tasar(oferta)
            hechas += 1
            if not cotizaciones:
                continue
    ...
    async def ciclo(self, dominios: list[str], solo_outlet: bool = False,
        ...
        self.detector.reiniciar_diagnostico()
        oportunidades: list = []
        n_lecturas = 0
        n_evaluadas = 0
        ...
        async def procesar(lote, cambiadas):
            ...
            # PRIMERO se averigua cuánto valen las que pintan bien, y DESPUÉS
            # se evalúan. Al revés no funciona: para pasar el filtro de margen
            # hace falta saber la reventa, así que si solo se consulta la
            # reventa de lo que ya pasó el filtro, no se consulta nunca nada.
            if presupuesto > 0:
                pretendientes = self.detector.candidatas(
                    a_evaluar, contexto=lote, tope=presupuesto)
                if pretendientes:
                    presupuesto -= await self.tasar_candidatas(
                        pretendientes, tope=presupuesto)
            ...
            oportunidades.extend(
                self.detector.evaluar_lote(a_evaluar, contexto=lote))
        ...
        # Cómo fue el ciclo, para que la web pueda explicar una página vacía.
        diag = self.detector.diagnostico()
        diag["lecturas"] = n_lecturas
        diag["evaluadas"] = n_evaluadas
        diag["tiendas"] = len(dominios)
        diag["minutos"] = round((time.monotonic() - arranque) / 60, 1)
        self.db.guardar_diagnostico(diag)
        errores = self.db.errores_frecuentes()
        diag["errores"] = errores
        self.db.guardar_diagnostico(diag)
        if errores:
            print("[tiendas] por qué fallan: " + " · ".join(
                f"{e['n']} {e['error'][:60]} (p.ej. {e['ejemplo']})"
                for e in errores))
        emb = diag.get("embudo", {})
        print("[embudo] " + " · ".join(
            f"{k.lstrip('*')}: {v:,}" for k, v in sorted(
                emb.items(), key=lambda kv: -kv[1])[:6]) or "[embudo] sin datos")
